...
/// - Stats cards row (Total Shipments, Total Boxes, Total Pallets, Shipped Items)
/// - Syncfusion DataGrid with columns (Outbound ID, Customer, Contact, Status, Boxes, Pallets, Time tracking)
...
class OutboundTimeTrackingSession {
  OutboundTimeTrackingSession({
    required this.id,
    required this.date,
  ...
  OutboundTimeTrackingSession copyWith({
    String? id,
    DateTime? date,
    DateTime? startTime,
    DateTime? endTime,
    ...
    return OutboundTimeTrackingSession(
      id: id ?? this.id,
      date: date ?? this.date,
...
class OutboundData {
  OutboundData({
    ...
    List<OutboundTimeTrackingSession>? timeTrackingSessions,
  ...
  final int boxes;
  final int pallets;
  final List<OutboundTimeTrackingSession> timeTrackingSessions;
...
/// Provider for outbound data with time tracking state
final outboundDataProvider = StateNotifierProvider<OutboundDataNotifier, List<OutboundData>>((ref) {
  return OutboundDataNotifier();
});

class OutboundDataNotifier extends StateNotifier<List<OutboundData>> {
  OutboundDataNotifier() : super([
    OutboundData(
      outboundId: '2001',
      customer: 'Amazon FBA',
      contact: 'Jane Smith',
  ...
  void startTimer(String outboundId) {
    state = state.map((outbound) {
      if (outbound.outboundId == outboundId) {
        // Stop any currently active session first
        final updatedSessions = outbound.timeTrackingSessions.map((s) {
          if (s.isActive) {
        ...
        updatedSessions.add(OutboundTimeTrackingSession(
        ...
        return OutboundData(
          ...
          boxes: outbound.boxes,
          pallets: outbound.pallets,
  ...
  void stopTimer(String outboundId) {
    state = state.map((outbound) {
      if (outbound.outboundId == outboundId) {
        final updatedSessions = outbound.timeTrackingSessions.map((s) {
          if (s.isActive) {
  ...
  void addManualSession(String outboundId, DateTime startTime, DateTime endTime) {
    state = state.map((outbound) {
      if (outbound.outboundId == outboundId) {
        final updatedSessions = List<OutboundTimeTrackingSession>.from(outbound.timeTrackingSessions);
        updatedSessions.add(OutboundTimeTrackingSession(
          id: DateTime.now().millisecondsSinceEpoch.toString(),
          date: startTime,
          startTime: startTime,
  ...
  void deleteSession(String outboundId, String sessionId) {
    state = state.map((outbound) {
      if (outbound.outboundId == outboundId) {
        final updatedSessions = outbound.timeTrackingSessions
  ...
  void clearSessions(String outboundId) {
    state = state.map((outbound) {
      if (outbound.outboundId == outboundId) {
        return OutboundData(
          outboundId: outbound.outboundId,
...
class _OutboundScreenState extends ConsumerState<OutboundScreen> {
  ...
  void initState() {
    ...
    _refreshTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (mounted) {
        ref.read(outboundDataProvider.notifier).refresh();
      }
    });
  }
  ...
  void _showTimeTrackingModal(OutboundData outbound) {
    showDialog(
      context: context,
      builder: (context) => OutboundTimeTrackingModal(outbound: outbound),
    );
  }

  @override
  Widget build(BuildContext context) {
    final outbounds = ref.watch(outboundDataProvider);
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  // Stats cards row
                  _buildStatsCards(outbounds),
                  const SizedBox(height: 24),
                  // Data grid section
                  _buildDataGridSection(outbounds),
                ],
              ),
            ),
          ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Track and manage outgoing shipments - 1 shipments loaded',
                        style: TextStyle(
                          fontSize: 14,
  ...
  Widget _buildStatsCards(List<OutboundData> outbounds) {
    final totalBoxes = outbounds.fold<int>(0, (sum, o) => sum + o.boxes);
    final totalPallets = outbounds.fold<int>(0, (sum, o) => sum + o.pallets);
  ...
  Widget _buildDataGridSection(List<OutboundData> outbounds) {
    final dataSource = OutboundDataSource(
      outbounds: outbounds,
      onTimeTrackingTap: _showTimeTrackingModal,
      onStartTimer: (outboundId) => ref.read(outboundDataProvider.notifier).startTimer(outboundId),
      onStopTimer: (outboundId) => ref.read(outboundDataProvider.notifier).stopTimer(outboundId),
    );
            ...
            child: SfDataGridTheme(
              ...
              child: SfDataGrid(
                ...
                columns: [
                  ...
                  // Outbound ID column
                  GridColumn(
                    columnName: 'outboundId',
                    minimumWidth: 120,
                    label: _buildColumnHeader('Outbound ID'),
                  ),
                  // Customer column
                  GridColumn(
                    columnName: 'customer',
...
/// Data source for Outbound DataGrid
class OutboundDataSource extends DataGridSource {
  final List<OutboundData> outbounds;
  final Function(OutboundData) onTimeTrackingTap;
  ...
  void buildDataGridRows() {
    _dataGridRows = outbounds.map<DataGridRow>((outbound) {
      return DataGridRow(cells: [
        const DataGridCell<Widget>(columnName: 'expand', value: null),
        DataGridCell<String>(columnName: 'outboundId', value: outbound.outboundId),
        DataGridCell<String>(columnName: 'customer', value: outbound.customer),
        DataGridCell<String>(columnName: 'contact', value: outbound.contact),
        DataGridCell<String>(columnName: 'status', value: outbound.status),
        DataGridCell<int>(columnName: 'boxes', value: outbound.boxes),
        DataGridCell<int>(columnName: 'pallets', value: outbound.pallets),
        DataGridCell<OutboundData>(columnName: 'timeTracking', value: outbound),
      ]);
    }).toList();
  }
    ...
    return DataGridRowAdapter(
      ...
      cells: row.getCells().map<Widget>((cell) {
        ...
        if (cell.columnName == 'timeTracking') {
          final outbound = cell.value as OutboundData;
          final isActive = outbound.hasActiveSession;
          final totalDuration = outbound.totalDuration;
            ...
            child: Row(
              children: [
                ...
                GestureDetector(
                  onTap: () {
                    if (isActive) {
                      onStopTimer(outbound.outboundId);
                    } else {
                      onStartTimer(outbound.outboundId);
                    }
                  },
                  child: Container(
                    width: 28,
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () => onTimeTrackingTap(outbound),
                    child: MouseRegion(
...
class OutboundTimeTrackingModal extends ConsumerStatefulWidget {
  final OutboundData outbound;

  const OutboundTimeTrackingModal({super.key, required this.outbound});

  @override
  ConsumerState<OutboundTimeTrackingModal> createState() => _OutboundTimeTrackingModalState();
}

class _OutboundTimeTrackingModalState extends ConsumerState<OutboundTimeTrackingModal> {
  Timer? _refreshTimer;
                ...
                GestureDetector(
                  onTap: () {
                    ...
                    if (endDateTime.isAfter(startDateTime)) {
                      ref.read(outboundDataProvider.notifier).addManualSession(
                        widget.outbound.outboundId,
                        startDateTime,
                        endDateTime,
                      );
  ...
  Widget build(BuildContext context) {
    ...
    final outbounds = ref.watch(outboundDataProvider);
    final currentOutbound = outbounds.firstWhere(
      (o) => o.outboundId == widget.outbound.outboundId,
      orElse: () => widget.outbound,
    );
    final sessions = currentOutbound.timeTrackingSessions;
    final isActive = currentOutbound.hasActiveSession;
    final totalDuration = currentOutbound.totalDuration;

    // If compact view (default)
    if (!_showFullCalendar) {
      return _buildCompactView(currentOutbound, sessions, isActive, totalDuration);
    }

    // Full calendar view
    return _buildFullCalendarView(currentOutbound, sessions, isActive, totalDuration);
  }

  /// Compact view - shows timer controls, total time, and expand button
  Widget _buildCompactView(OutboundData outbound, List<OutboundTimeTrackingSession> sessions, bool isActive, Duration totalDuration) {
    return Dialog(
      backgroundColor: AppColors.card,
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          if (isActive) {
                            ref.read(outboundDataProvider.notifier).stopTimer(widget.outbound.outboundId);
                          } else {
                            ref.read(outboundDataProvider.notifier).startTimer(widget.outbound.outboundId);
                          }
                        },
                        child: Container(
                          width: 56,
  ...
  Widget _buildFullCalendarView(OutboundData outbound, List<OutboundTimeTrackingSession> sessions, bool isActive, Duration totalDuration) {
    return Dialog(
      backgroundColor: AppColors.card,
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
              ...
              children: [
                ...
                Row(
                  children: [
                    GestureDetector(
                      onTap: () {
                        ref.read(outboundDataProvider.notifier).clearSessions(widget.outbound.outboundId);
                      },
                      child: const Text(
                        'Clear',
                        style: TextStyle(
              ...
              ConstrainedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _buildCleanSessionItem(session, outbound.outboundId);
                  },
                ),
              ),
            const SizedBox(height: 16),
  ...
  Widget _buildCleanSessionItem(OutboundTimeTrackingSession session, String outboundId) {
      ...
      child: Row(
        children: [
          ...
          if (!session.isActive) ...[
            ...
            GestureDetector(
              onTap: () {
                ref.read(outboundDataProvider.notifier).deleteSession(outboundId, session.id);
              },
              child: Icon(
